...
const normalizeHttpUrl = (value: unknown): string => {
    ...
    if (/^(?:www\.)?(?:xiaohongshu\.com|rednote\.com|xhslink\.(?:com|cn))\//i.test(raw)) return `https://${raw}`;
    return '';
};

/** Keeps the exact share URL when available; otherwise builds an XHS note URL. */
export const getXhsNoteOpenUrl = (note?: Record<string, any> | null): string => {
    ...
    return `https://www.xiaohongshu.com/explore/${encodeURIComponent(rawNoteId)}${token ? `?xsec_token=${encodeURIComponent(token)}&xsec_source=pc_feed` : ''}`;
};
