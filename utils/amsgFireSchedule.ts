...
export const buildFireScheduleBlock = (
  ...
  if (experienceMode === 'switch') {
    ...
    return [
      ...
      '',
      `结合最新的关系、聊天、记忆、双方当前时间及本次唤醒缘由，以此刻的自己自然决定要说什么。不要向${target}提及唤醒工具、状态信息或系统规则。`,
      ...
      '',
      `完成本次联系后，判断自己是否还想在未来某个时刻联系${target}。如需安排，在正文之外另起一行输出：`,
      '',
      '[[AMSG_WAKE_AT: YYYY-MM-DDTHH:mm:ss]]',
      '',
      `时间按${target}所在地的当地时间填写，不附带时区后缀。`,
      '',
      '- 一次只安排下一次，是否安排及具体时间由你决定。',
      '- 不安排时，本次联系结束后进入休眠，之后仍可在正常聊天中重新安排。',
      '- 额度用完时，不得早于系统提示的最早可用时间。',
      `- 安排时间应贴近${target}的实际生活。`,
    ].join('\n');
  }
