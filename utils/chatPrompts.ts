...
export const ChatPrompts = {
    ...
    buildSystemPromptParts: async (
        ...
        const groupContextPromise: Promise<string> = (async () => {
            try {
                ...
                const groupLogStr = recentGroupMsgs.map(m => {
                    ...
                    return `[${dateStr}${relativeAge}] [群：${m.groupName}] ${speaker}: ${summarizeGroupMsgContent(m)}`;
                }).join('\n');
        ...
        //     worker 两样都够不着。
        if (!isCodeSurface && !forFirePack) try {
            let charListening: {
...
但务必记住：这只是 ta 挂在虚拟空间里的一个化身状态（类似游戏挂机 / AFK），**并不代表 ${uname} 本人此刻真守在游戏里**——ta 很可能早已离开屏幕、正在现实里忙别的或休息。所以别据此认定"ta 正盯着你""ta 现实里也在干这件事"，也别把它当成 ta 在跟你说话。你和 ta 的真实关系、近况一律以你们的聊天记录为准；这条只是彼方这个虚拟空间里的一个在场提示而已。\n`;
        ...
        const phoneXhsAvailable = !!(
            !forFirePack
            && realtimeConfig?.xhsPhoneConfig?.enabled
...
这里是${userProfile.name}邀请你一起使用编程 AI 的 Code 对话。普通聊天记录只作为关系、情绪和语气背景；技术细节、决策和待办以当前 Code 对话为准。其他 Code 对话的进度卡只作索引，除非对方明确提到，否则不要混入当前任务。

保持你的性格与 IM 口语风格，不输出行为、时间戳、名字前缀或系统日志。不要使用语音、字幕、电话口吻和音乐动作。多条消息用真正的换行分隔。

普通回复不得生成、续写或模仿“[Code 进度]”格式；仅在系统明确要求总结进度卡时使用。用户讨论任务、Prompt、进度或总结不视为该要求。

你可以使用表情包、引用、@Codex，以及当前已启用的小红书工具；不要使用其他普通聊天专属工具。

${xhsEnabled || xhsPhoneEnabled ? `### 小红书
当前 Code 对话已启用你的小红书能力。可以使用后文提供的搜索和浏览指令；分享搜索或浏览结果时使用 \`[[XHS_SHARE:N]]\`，系统会发送小红书卡片。

` : ''}- 表情包：\`[[SEND_EMOJI:表情名称]]\`
- 引用：\`[[QUOTE:引用内容]]\`
- 需要 Codex/CLI 接手技术任务：在回复末尾单独使用 \`[[MENTION_CODEX]]\`
...
### 你知道「协同工作」功能
你现在在普通聊天窗口。你知道自己可以和用户进入独立的「协同工作」区域办正事；遇到需要制作、修改或整理文件的需求，可以自然地引导用户从 ChatApp 加号页进入那里。不要在普通聊天里执行或假装执行这些工作。
系统会另行告诉你当前文件柜里有哪些已有文件以及哪些正文实际可读；你可以把清单中的已有文件发给用户。没有清单时不要猜文件。
`;
        }
