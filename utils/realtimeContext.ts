...
export const RealtimeContextManager = {
    ...
    buildFullContext: async (
        ...
        const worldContext = renderRealtimeWorldBlock({ timeLine, specialDates, weather, news: picks });
        const deviceContext = `user当前使用设备：${detectUserDeviceInfo()}`;
        const fullContext = [worldContext, deviceContext].filter(Boolean).join('\n');
