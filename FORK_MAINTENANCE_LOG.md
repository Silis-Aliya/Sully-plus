...
- Extended the existing per-character video-call stage background without breaking the legacy single-background field. The Video Stage Tools sheet now offers `Single background` and `Time mode`.
- Time mode offers 3/4/5/6-segment presets and retains the images belonging to hidden segments when switching presets. The six-slot vocabulary is morning, noon, afternoon, dusk, evening, and night; local images continue through the IndexedDB `blobref` pipeline and therefore participate in full backup/restore.
- Background selection uses the called character's configured IANA timezone, not the device clock. Each preset displays its exact hour ranges in the picker, and a live video call checks every minute so crossing a boundary changes the stage automatically.
- Reworked the time-background picker into a one-at-a-time accordion. It opens the character's current period by default; collapsed rows retain the period, exact range, current-period marker, and configured state. Light mode now uses explicit white and soft gray-blue surfaces so the Call compatibility theme cannot invert these controls into dark blocks.
- Added an independent transparent foreground layer above the Live2D/VRM/static character for desk edges, chair backs, and other occlusion props. It supports a fixed image or the same character-timezone 3/4/5/6-period schedule as backgrounds. The user can enter a deliberate layout mode to drag and scale the foreground, then lock it with `Confirm position`; normal calls keep the layer pointer-transparent so it cannot move accidentally.
- Foreground images, schedule, mode, and locked placement follow the existing image persistence contract: IndexedDB Blob refs, complete/media backup, character export/import migration, and QuickSync nested Blob discovery.
- An unconfigured time slot falls back to the character's legacy single background, avoiding a blank stage during gradual setup. Replacing or clearing one slot only deletes an old local Blob when no other slot or fallback still references it.
...
- Replaced the full-width green suspended-call strip with the same bare animated Chibi figure used in VR World, rather than wrapping it in another circular avatar ball. The figure floats, drags, persists its position, and resumes the call on tap; the redundant green phone-status badge was removed so only the character figure remains. It reads the authoritative shared `getChibi` path and does not substitute Room/legacy Chibi art or run a second screenshot renderer.
