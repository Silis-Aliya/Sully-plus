...
- Replaced the full-width green suspended-call strip with an independent draggable call bubble using the character's VR World Chibi (falling back through the shared portrait/avatar resolver). Its position persists, tapping resumes the suspended call, and it remains separate from the music mini-player.
- Extended the suspended-call Chibi path for migrated workshop data: `vrState.chibi.img` remains authoritative, followed by `chibiStudio.vr.img`. When only the saved VR face-part state survives, a hidden creator renderer reconstructs that exact state into a transparent preview; unrelated Room/legacy `sprites.chibi` images are deliberately not used as the VR avatar. Ordinary portraits and avatars remain only a temporary/final fallback when no VR state exists.
