...
- Replaced the full-width green suspended-call strip with the same bare animated Chibi figure used in VR World, rather than wrapping it in another circular avatar ball. The figure floats, drags, persists its position, resumes the call on tap, and carries only a small phone-status badge separate from the music player. It reads the authoritative shared `getChibi` path and does not substitute Room/legacy Chibi art or run a second screenshot renderer.
