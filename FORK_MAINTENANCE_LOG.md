...
- Extended the shared VR Chibi resolver for migrated workshop data: `vrState.chibi.img` remains authoritative, followed by `chibiStudio.vr.img` and legacy non-Blob `sprites.chibi`; ordinary portraits and avatars are now only the final fallback. This prevents the suspended-call bubble from showing the normal character avatar when a compatible VR workshop image exists.
