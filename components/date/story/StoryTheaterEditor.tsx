    ...
    return <div className='h-full w-full flex flex-col bg-stone-100 text-slate-800'>
        ...
        <main className='story-page-scroll flex-1 overflow-y-auto px-5 py-6 pb-28'><div className='max-w-2xl mx-auto space-y-8'>
            ...
            <section className='pt-6 border-t border-slate-200'>
                ...
                <div className='mt-5 pt-4 border-t border-slate-200'><div className='flex items-start justify-between gap-5'><div><div className='text-sm font-semibold'>不发送高级采样参数</div><p className='mt-1 text-[10px] leading-5 text-slate-500'>默认关闭。仅当接口不接受这些字段时开启；开启后不发送 top_p、frequency_penalty 和 presence_penalty。</p></div><Toggle label='不发送高级采样参数' value={draft.omitSamplingParams === true} onChange={value => update('omitSamplingParams', value)} /></div></div>
