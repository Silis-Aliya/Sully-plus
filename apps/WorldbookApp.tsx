...
const WorldbookApp: React.FC = () => {
    const { closeApp, worldbooks, characters, addWorldbook, updateWorldbook, deleteWorldbook, updateCharacter, addToast } = useOS();
    ...
    const confirmBulkDelete = async () => {
        ...
        for (const char of characters) {
            if (!char.mountedWorldbooks?.some(book => deletedIds.has(book.id))) continue;
            updateCharacter(char.id, current => ({
                mountedWorldbooks: current.mountedWorldbooks?.filter(book => !deletedIds.has(book.id)) || [],
            }));
        }
