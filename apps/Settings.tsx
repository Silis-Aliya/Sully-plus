...
const Settings: React.FC = () => {
  ...
  const { characters, groups, userProfile } = useCharacterData();
  const {
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 no-scrollbar pb-20">
        ...
        >
            <p className="text-[11px] text-slate-400 mb-4 leading-relaxed pl-1">
                语音生成 / 写歌等非 LLM 类 API。这些设置 <span className="font-semibold text-slate-500">不会随预设切换</span>，通常只配置一次。
            </p>
        ...
        {/* 语音识别区域 — 从其他 API 拆出，避免语音教程撑满整页 */}
        <SettingsSection
            title="语音识别"
            icon={
                <div className="p-2 bg-sky-100/50 rounded-xl text-sky-600 overflow-visible">
        ...
        >
            <p className="text-[11px] text-slate-400 mb-4 leading-relaxed pl-1">
                录音转写、Ears Lite 本地声音特征、Groq 语气转述、腾讯云声纹和讯飞声音画像都在这里。语音生成仍在下面的「其他 API」。
            </p>
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        const guide = [
                                            ...
                                            '2. 回到 SullyOS 设置 → 语音识别，把完整 Groq API Key 填进去并保存。',
                                            '3. 之后手机/电脑都会走内置 Ears Lite：Essentia.js 在本机提取声音特征，Groq Whisper 做转写。',
                                        ].join('\n');
                                        try {
                                ...
                                <button
                                    ...
                                    onClick={async () => {
                                        const guide = [
                                            ...
                                            '4. 回到 SullyOS 设置 → 语音识别，把 APPID 填到讯飞声音特征识别里并保存。',
