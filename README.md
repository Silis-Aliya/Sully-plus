...
- 当前一起听会话会作为最长 12 小时有效的短时现场快照跨刷新、完整备份和增量同步恢复，包括参与角色、当前歌曲、队列、播放模式与角色下一次主动唤醒计划；播放进度秒数和播放/暂停状态不会恢复。恢复时只保留仍在一起听的角色计划，超过 10 分钟的过期唤醒不会补发。
...
- 一起听关系、当前歌曲、队列、播放模式和对应的角色主动唤醒计划作为带有效期的短时现场快照迁移；播放秒数、播放/暂停状态等纯瞬时状态不迁移。
...
- 一起听的参与角色、当前歌曲、队列、播放模式和对应的角色主动唤醒计划属于带有效期的短时现场快照；播放进度秒数、播放/暂停状态、API 调用日志和可重建缓存不属于用户存档。
